...
void cTexture::Load()
{
    if (!m_texture) return;//이미 텍스쳐가 있는거 아니였어...?

    if (D3DXGetImageInfoFromFile(m_filePath.c_str(), &m_info) != S_OK)
    {
        wclog << "cTexture::Load - Can not Get Image Info " << m_filePath << endl;
    }
    else if (D3DXCreateTextureFromFileEx(g_Device.GetDevice(), m_filePath.c_str(), D3DX_DEFAULT_NONPOW2, D3DX_DEFAULT_NONPOW2, m_info.MipLevels, 0, 
